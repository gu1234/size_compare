...
let successCount = parseInt(localStorage.getItem('distanceChallengeSuccessCount') || '0', 10);
...
function updateSuccessCounter() {
  ...
  localStorage.setItem('distanceChallengeSuccessCount', successCount.toString());
}
...
function nextRound() {
  ...
  showMessage('Which is closer to Earth? Tap to choose!');
}
...
function onClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    if (mesh && mesh.userData && mesh.userData.obj) {
      ...
      if (picked.obj.distanceFromEarth < other.distanceFromEarth) {
        successCount++;
        updateSuccessCounter();
