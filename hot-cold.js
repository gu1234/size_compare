...
let successCount = parseInt(localStorage.getItem('hotColdSuccessCount') || '0', 10);
...
function updateSuccessCounter() {
  ...
  localStorage.setItem('hotColdSuccessCount', successCount.toString());
}
...
function nextRound() {
  if (objects.length < 2) {
    showMessage('Need at least two objects to play!', '#ffcc00');
    return;
  }
  canClick = true;
  currentPair = pickTwoObjects();
  showPair(currentPair);
  showMessage('Which is hotter? Tap to choose!');
}
...
function handlePick(index) {
  ...
  if (!picked.obj || !other || picked.obj.temperatureK == null || other.temperatureK == null) {
    showMessage('Missing temperature data. Try another pair.', '#ffcc00');
    setTimeout(() => {
      nextRound();
    }, 1200);
    return;
  ...
  if (picked.obj.temperatureK > other.temperatureK) {
    successCount++;
    updateSuccessCounter();
    ...
    showMessage(
      `Correct! ${picked.obj.name} is hotter (${formatTemp(picked.obj.temperatureK)} vs ${formatTemp(other.temperatureK)}).`,
      '#00ff99'
    );
    setTimeout(() => {
      nextRound();
    ...
    tryAgainSound.play();
    showMessage(
      `Try again! ${picked.obj.name} is cooler (${formatTemp(picked.obj.temperatureK)} vs ${formatTemp(other.temperatureK)}).`,
      '#ffcc00'
    );
    setTimeout(() => {
      canClick = true;
...
function onClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    if (mesh && mesh.userData && mesh.userData.obj) {
      canClick = false;
      handlePick(mesh.userData.index);
    }
  }
}
